...
#[tauri::command]
pub fn send_recognized_microphone(recognized: String, isFinal: bool) {
    let sock = UdpSocket::bind((Ipv4Addr::UNSPECIFIED, 0)).unwrap();
    let msg_buf = encoder::encode(&OscPacket::Message(OscMessage {
        addr: "/detection/microphone".to_string(),
        args: vec![OscType::String(recognized), OscType::Bool(isFinal)],
    }))
    .unwrap();
...
#[tauri::command]
pub fn send_recognized_desktop(recognized: String, isFinal: bool) {
    let sock = UdpSocket::bind((Ipv4Addr::UNSPECIFIED, 0)).unwrap();
    let msg_buf = encoder::encode(&OscPacket::Message(OscMessage {
